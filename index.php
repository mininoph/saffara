<?php
// ============================================================
// صفارة لايف - الصفحة الرئيسية (تعرض المباريات من API البانل)
// ============================================================
...
if ($cache_valid) {
    ...
    if ($result['success']) {
        ...
        $api_error        = true;
        $api_error_reason = $result['reason'];
...
foreach ($matches as $key => $match) {
    ...
    $time = new DateTime($match['match_datetime']);
  
    $matches[$key]['match_datetime_utc'] = $time->format('Y-m-d H:i:s');
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        main {
            position: relative; z-index: 5;
            max-width: 820px; margin: 22px auto 50px; padding: 0 20px;
        }
        ...
        .match-center {
            display: flex; flex-direction: column; align-items: center;
            gap: 5px; flex-shrink: 0; padding: 0 12px;
        }
        .vs-badge {
            width: 34px; height: 34px; border-radius: 50%;
            background: rgba(0,229,160,0.08); border: 1px solid var(--border-hover);
            display: flex; align-items: center; justify-content: center;
        ...
        .play-circle {
            ...
            background: rgba(5,8,16,0.22); border-radius: 50%;
            display: flex; align-items: center; justify-content: center;
            font-size: 10px; padding-right: 1px;
        }

        /* ── انتهت المباراة: لون أبرز ── */
        .finished-label {
...
.match-result {
    ...
    border-radius: 10px;
        ...
        .notice-bar-inner.warning {
            background: rgba(245,200,66,0.08);
            border: 1px solid rgba(245,200,66,0.22);
            color: var(--gold);
        }
        .notice-bar-inner.info {
        ...
        .footer-links a {
            ...
            transition: color 0.25s, padding-right 0.25s; display: inline-block;
        }
        ...
        @media (max-width: 700px) {
            .footer-inner { grid-template-columns: 1fr 1fr; gap: 24px 20px; padding: 24px 18px; }
            .content-inner { padding: 20px; }
            .content-title { font-size: 19px; }
        }
        @media (max-width: 450px) {
            .footer-inner { grid-template-columns: 1fr; gap: 20px; padding: 20px 16px; }
            .footer-col { text-align: center; }
            .footer-logo { justify-content: center; }
            .footer-links a:hover { padding-right: 0; }
            .content-features { justify-content: center; }
        }
    </style>
</head>
<body>
...
<div class="date-strip">
    <div style="display:flex; align-items:center; gap:10px; flex-wrap:wrap;">
        <a href="/" id="btn-today" style="padding:7px 18px; border-radius:30px; font-size:13px; font-weight:700; text-decoration:none; background:linear-gradient(135deg,#00e5a0,#00b87a); color:#050810;">مباريات اليوم</a>
        <a href="/tomorrow/" id="btn-tomorrow" style="padding:7px 18px; border-radius:30px; font-size:13px; font-weight:700; text-decoration:none; background:rgba(255,255,255,0.06); color:var(--text-sub); border:1px solid var(--border);">مباريات الغد</a>
    </div>
...
<?php if ($api_error && $using_cache && $cache_age !== null): ?>
    <div class="notice-bar">
        <div class="notice-bar-inner warning">
            ⚠️ تعذر الاتصال بالخادم — يتم عرض آخر بيانات متاحة
            (منذ <?php echo round($cache_age / 60); ?> دقيقة)
        </div>
    </div>
<?php elseif ($api_error && !$using_cache): ?>
    <div class="notice-bar">
        <div class="notice-bar-inner warning">
            ❌ تعذر تحميل المباريات — يرجى المحاولة لاحقاً
        </div>
    </div>
<?php endif; ?>
...
<main>
    <div class="matches-list">
        ...
        <?php elseif (count($matches) > 0): ?>
            <?php foreach ($matches as $match): ?>
                ...
                $match_time = new DateTime($match['match_datetime_original'], new DateTimeZone('UTC'));
                $diff       = $now->getTimestamp() - $match_time->getTimestamp();

                // حساب الـ slug
                $slug = urlencode($match['team1_name'] . '-vs-' . $match['team2_name'] . '-' . date('Y-m-d', strtotime($match['match_datetime'])));

                if ($diff < -900) {
                    // ── لم تبدأ بعد (أكثر من 15 دقيقة) ──
                    $status      = 'upcoming';
                   ...
                   $watch_link = '/match.php?slug=' . $match['id'];
                    ...
                    // ── جارية الآن (15 دقيقة قبل → ساعتين بعد) ──
                    $status      = 'live';
    ...
    $watch_link  = null;
    $btn_label   = null;
    // ✅ جلب النتيجة من بيانات المباراة
    $team1_score = isset($match['team1_score']) ? $match['team1_score'] : '?';
    $team2_score = isset($match['team2_score']) ? $match['team2_score'] : '?';
}
                ?>
                <div class="match-card"
                     data-match-time="<?php echo $match['match_datetime_utc']; ?>"
                     data-match-id="<?php echo $match['id']; ?>"
                     data-match-slug="/match/<?php echo $slug; ?>/">
    ...
    <?php if ($status === 'finished'): ?>
        <div class="match-result">
            <?php echo $team1_score; ?> - <?php echo $team2_score; ?>
        </div>
    <?php else: ?>
        <div class="match-time-display match-time-local<?php echo $show_time ? '' : ' hidden'; ?>"
    ...
    <?php endif; ?>
        ...
        <?php else: ?>
            <div class="no-matches">
                ...
                <p>لا توجد مباريات مجدولة اليوم</p>
            </div>
        <?php endif; ?>
...
<section class="content-section">
    <div class="content-inner">
        <h2 class="content-title">مرحباً بك في صفارة لايف</h2>
        <p class="content-desc">
            صفارة لايف هي منصتك الأولى لمشاهدة المباريات بث مباشر بجودة عالية 
            بدون تقطيع وبأفضل خدمة. نوفر لك جميع المباريات المحلية والعالمية 
            مع تحديثات فورية ومتابعة دقيقة لكل البطولات.
        </p>
        <div class="content-features">
            <span class="feature-tag">⚡ بث مباشر HD</span>
            <span class="feature-tag">📺 جميع البطولات</span>
            <span class="feature-tag">🎙️ تعليق عربي</span>
            <span class="feature-tag">📱 متوافق مع جميع الأجهزة</span>
        </div>
    </div>
</section>
...
setInterval(updateClock, 1000);
    ...
    function updateMatchStatus() {
        ...
        document.querySelectorAll('.match-card').forEach(function(card) {
            ...
            var matchSlug    = card.getAttribute('data-match-slug');
            if (!matchTimeStr) return;

            // إضافة Z لأن الوقت UTC
            ...
            if (diffSeconds < -900) {
              ...
              footerEl.innerHTML =
              '<a href="/match.php?slug=' + card.getAttribute('data-match-id') + '" class="watch-btn">' +
              '<span class="play-circle">▶</span>شاهد المباراة</a>';
                ...
                footerEl.innerHTML =
                    '<a href="/streams/generate.php?match_id=' + matchId + '" class="watch-btn">' +
                    '<span class="play-circle">▶</span>شاهد الآن</a>';
